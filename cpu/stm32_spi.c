...
static mm_u32 spi_frame_bytes(const struct stm32_spi_inst *s)
{
...
static mm_u8 fifo_count(const struct stm32_spi_inst *s)
{
    return (mm_u8)((s->rx_tail - s->rx_head) & 0x1Fu);
}

static mm_bool fifo_push(struct stm32_spi_inst *s, mm_u8 v)
{
...
static mm_bool fifo_pop(struct stm32_spi_inst *s, mm_u8 *out)
{
...
static mm_u32 compute_sr(const struct stm32_spi_inst *s)
{
    mm_u32 sr = 0;
...
static void update_sr(struct stm32_spi_inst *s)
{
    s->regs[SPI_SR / 4] = compute_sr(s);
}

static void spi_bus_poll_if_needed(struct stm32_spi_inst *s)
{
...
static void spi_set_enabled(struct stm32_spi_inst *s, mm_bool enable)
{
    if (enable) {
        s->enabled = MM_TRUE;
    } else {
...
static void spi_start_transfer(struct stm32_spi_inst *s)
{
...
static mm_u8 spi_xfer_byte(struct stm32_spi_inst *s, mm_u8 out)
{
    spi_bus_poll_if_needed(s);
    return mm_spi_bus_xfer(s->bus_index, out);
}

static void spi_handle_tx(struct stm32_spi_inst *s, mm_u32 value, mm_u32 size_bytes)
{
    mm_u32 i;
...
{
    struct stm32_spi_inst *s = (struct stm32_spi_inst *)opaque;
    mm_u32 val = 0;
    mm_u8 *out = (mm_u8 *)dst;
...
static mm_bool spi_load(void *opaque, struct mm_snapshot_reader *r)
{
    ...
    memcpy(s->regs, snap.regs, sizeof(s->regs));
...
static void poll_instance(struct stm32_spi_state *state, struct stm32_spi_inst *s)
{
    mm_u32 ier;
    mm_u32 sr;
    if (!s->enabled) return;
    ...
    sr = s->regs[SPI_SR / 4];
    if (state->nvic == 0 || s->irq < 0) return;
    if ((ier & IER_RXPIE) && (sr & SR_RXP)) {
        mm_nvic_set_pending(state->nvic, (mm_u32)s->irq, MM_TRUE);
    }
...
void stm32_spi_poll(struct stm32_spi_state *state)
{
    ...
    for (i = 0; i < state->spi_count; ++i) {
        poll_instance(state, &state->spis[i]);
    }
}
                    ...
                    struct mm_nvic *nvic)
{
    ...
    state->cfg = cfg;
    state->nvic = nvic;
    state->spi_count = cfg->count;
    if (state->spi_count > STM32_SPI_MAX_INSTANCES) {
        state->spi_count = STM32_SPI_MAX_INSTANCES;
    }
    for (i = 0; i < state->spi_count; ++i) {
        struct stm32_spi_inst *s = &state->spis[i];
        ...
        s->irq = cfg->irq_map != 0 ? cfg->irq_map[i] : -1;
        s->name = cfg->names != 0 ? cfg->names[i] : 0;
...
void stm32_spi_reset(struct stm32_spi_state *state)
{
    ...
    for (i = 0; i < STM32_SPI_MAX_INSTANCES; ++i) {
        memset(&state->spis[i], 0, sizeof(state->spis[i]));
    }
    state->spi_count = 0u;
    state->nvic = 0;
    state->cfg = 0;
