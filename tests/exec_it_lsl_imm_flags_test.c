...
int main(void)
{
    static const mm_u8 code[] = {
        0x00, 0x20, /* movs r0, #0    */
        0x47, 0x21, /* movs r1, #0x47 */
        0x3d, 0x29, /* cmp  r1, #0x3d */
        0x18, 0xbf, /* it   ne        */
        0x00, 0x01, /* lslne r0, r0, #4 */
        0x18, 0xbf, /* it   ne        */
        0x01, 0x22, /* movne r2, #1   */
    };
    struct mm_cpu cpu;
    size_t i;
    ...
    if (run_sequence(code, sizeof(code), &cpu) != 0) {
        printf("exec_it_lsl_imm_flags_test: sequence execution failed\n");
        return 1;
    }

    if (cpu.r[2] != 1u) {
        printf("exec_it_lsl_imm_flags_test: MOVNE after LSLS-in-IT did not execute, r2=0x%08lx\n",
